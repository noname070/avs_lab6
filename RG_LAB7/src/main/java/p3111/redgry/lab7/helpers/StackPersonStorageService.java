...
import p3111.redgry.lab7.utils.CsvReader;
...
public class StackPersonStorageService implements StorageService {
    ...
    public boolean removeById(long id) {
        Map<Long, Person> personToDelete = st.toList();
    ...
    public List<String> removeGreaterKey(long key) {
         ...
         * CommandsManager.getInstance().printToClient("Персонаж с ключем: " +
         * listKey.toArray()[i] + " удален.");
         * }
         * }
         */
        return listKey;
     ...
     * @param person обьект.
     * @return True - обновление выполнено, False - данного элемента нет в
     *         коллекции.
     */
    @Override
    ...
    /**
     * Возвращает кол-во элементов в коллекции, которые меньше чем произведение
     * X,Y,Z Локации экзепляра.
     * 
    ...
    public String CountLessThanLocation(double v) {
        Integer count = 0;
        StringBuilder sb = new StringBuilder(
                "Элементов в коллекции меньше по произведению координат локации равному " + v + ": ");
        StringBuilder sb2 = new StringBuilder();
        st.toList()
                .forEach((key, person) -> sb2.append(
                        person.getLocation().getX() * person.getLocation().getY() * person.getLocation().getZ() < v)
                        .append(";"));
     ...
     * @throws IOException Пробрасывается от команды, в случае если команда работает
     *                     с I/O и произошла ошибка.
     */
    @Override
    public void save(String pathToFile) throws IOException {
        try (OutputStreamWriter fw1 = new OutputStreamWriter(new FileOutputStream(pathToFile));
                CSVWriter writer = new CSVWriter(fw1, CsvReader.getDelimiter().charAt(0), '"', '"', "\n")) {
            ...
            // System.out.print("");
            log.error("Ошибка при сохранении коллекции", e);
        }
    }
    ...
    public String display() {
        Map<Long, Person> personList = st.toList();
        List<Long> listKey = new ArrayList<>(st.getPersons().keySet());
        StringBuilder sb = new StringBuilder();
    ...
    @Override
    public void addAll(Map<Long, Person> map) {
        st.putAll(map);
    }
