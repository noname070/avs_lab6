    ...
    public ArrayList<String> execute(
        ...
        if (args.length == argumentsCount) {
            try {
                ...
                if (ss.checkKey(id)) {
                    getPerson().setId(id);
                    dataBaseManager.addElementToDb(getPerson());
                    if (ss.removeGreaterKey(id) != null) {
                        ...
                        CommandsManager.getInstance().printToClient(
                                "Персонажи превышающие ключ: " + id + " и пренадлежащие вам успешно удалены!");
                    } else {
                        CommandsManager.getInstance()
                                .printToClient("В коллекции нет персонажей превышающих ключ: " + id + ".");
                    }
                    DataBaseManagerv2.updateCollectionFromDataBase(ss);
                } else
                    CommandsManager.getInstance().printToClient("Персонаж уже существует с данным ID.");
                return null;
            } finally {
